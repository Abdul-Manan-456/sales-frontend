...
const AddEditSales: React.FC<AddCategoryProps> = ({ mutate }) => {
    ...
    const onSubmit = async (data: any) => {
        ...
        setLoading(true)
        await axios.post(`${baseUrl}/invoice`, submittedData, {
            withCredentials: true
        }).then(() => {
            setLoading(false)
            mutate
    ...
    return (
        <div className="bg-slate-100 py-10 flex justify-center w-full">
            <Card className="rounded-sm w-full max-w-xs sm:max-w-4xl">
                <CardHeader>
                    <Button onClick={router.back} className="h-8 w-20 mb-2"> <BackArrow /> Back</Button>
                ...
                <CardContent>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div>
                            ...
                            <Select {...register("userId")} onValueChange={(value) => setValue("userId", value)}>
                                ...
                                <SelectContent>
                                    <SelectGroup>
                                        ...
                                        {customers && customers.map((customer: any) => (<SelectItem className="capitalize" key={customer._id} value={customer._id}>
                                            {customer.name}
                                        </SelectItem>
                                        ))}
                                    </SelectGroup>
                            ...
                            <p className="text-xs text-red-500">{errors && errors?.userId?.message}</p>
                        </div>
                        ...
                        <div>
                            {
                                fields.map((item, index) => (
                                    <div key={item.id} className="flex items-start gap-3 flex-row border pt-3 px-3 bg-slate-100 rounded-md my-6">
                                        <div className="w-full sm:flex sm:space-x-3 sm:justify-between mb-1">
                                            ...
                                            <div className="relative mt-4 sm:w-44 w-full">
                                                ...
                                                {errors.items && errors.items[index] && errors.items[index].category && (
                                                    <span className="text-xs absolute text-rose-500 top-1 right-0">{errors.items[index].category.message}</span>
                                                )}
                                            </div>
                                            ...
                                            <div className="relative mt-4 sm:w-44 w-full">
                                                ...
                                                {errors.items && errors.items[index] && errors.items[index].subCategory && (
                                                    <span className="text-xs absolute text-rose-500 top-1 right-0">{errors.items[index].subCategory.message}</span>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex items-center mt-6 space-x-4 justify-between mb-4 sm:mb-2">
                                                ...
                                                {!!index ? <DeleteIcon type="button" onClick={() => remove(index)} className="text-gray-500 w-9 h-7 cursor-pointer" /> : <div></div>

                                                }
                                                <MaterialSymbolsAdd type="button" onClick={appendItem} className="bg-accent rounded-sm text-accent-foreground w-9 h-7 cursor-pointer" />
                                            </div>
                                        </div>
                                    </div>
                                ))
                        ...
                        <div>
                            ...
                            <div className="flex items-center space-x-2 mb-4 mt-4">
                                <Switch {...register("status")} onCheckedChange={() => setIsChecked(!isChecked)} id="unPaid" />
                        ...
                        <div className="mt-10 flex items-center justify-between">
                            <Link href="/sales">
                                <Button variant={"outline"} className="hover:bg-gray-700">Cancel</Button>
                            </Link>
                            <Button className="h-10 px-8" disabled={loading} type="submit">Save Changes<span className={loading ? "loader ml-4" : ""}></span></Button>
                        </div>
                    </form>
                </CardContent>
