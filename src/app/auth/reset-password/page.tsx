...
const ResetPassword = () => {
    ...
    const onSubmit = async (data: any) => {
        setLoading(true)
        setError('')
        const resetData = { password: data.password, resetPasswordToken: param as string }
        await axios
            .post("http://localhost:4000/api/v1/auth/reset-password", resetData)
            .then(() => {
                setLoading(false);
                    ...
                    >
                        <div>
                            ...
                            <div className="w-full flex flex-col items-center justify-center">
                                <Button disabled={loading} type="submit" className=" mt-6 rounded-sm w-full">Reset Password<span className={loading ? "loader ml-3" : ""}></span></Button>

                                {
                                    error &&
                                    <Link href={'/auth/forgot-password'} className="text-sm underline mt-5">Verify Your Mail</Link>
                                }
